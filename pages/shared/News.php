            ...
            <?php while ($news->fetch()): ?>
                <div class="w-full max-w-sm mx-auto flex items-center flex-col rounded-md shadow-md overflow-hidden">
                    ...
                    <?php if(isset($_SESSION['student_num'])) : ?>           
                        <button onclick="window.location.href='<?= BASE_PATH ?>s/comment?news_id=<?=$newsId?>';" class="group my-6 relative h-12 w-48 overflow-hidden rounded-lg bg-white text-lg shadow">
                            <div class="absolute inset-0 w-3 bg-amber-400 transition-all duration-[250ms] ease-out group-hover:w-full"></div>
